    ...
    const resultado = usuarios.map((u) => {
      const rolPrincipal =
        u.instituciones
          .map((i) => i.rol.nombre)
          .sort(
            (a, b) => (ROL_PRIORIDAD[b] || 0) - (ROL_PRIORIDAD[a] || 0),
          )[0] || null
...
const crearUsuario = async (req, res) => {
  try {
    ...
    const setupTokenHash = crypto
      ...
      .update(setupToken)
      .digest('hex')
    const setupTokenExpires = new Date(Date.now() + 48 * 60 * 60 * 1000) // 48 horas
    ...
    // Notificar al nuevo usuario con enlace para establecer contraseña
    enviarEmailBienvenida({ email, nombre, setupToken, rol }).catch((err) =>
